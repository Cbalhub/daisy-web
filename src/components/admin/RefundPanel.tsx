...
export function RefundPanel({ paymentId }: { paymentId: string }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (!res.ok) {
      ...
      const message = body?.error ?? "환불 처리에 실패했습니다.";
      setError(message);
      toast(message, "error");
      setLoading(false);
      return;
  ...
  return (
    <form onSubmit={submit} className="rounded-lg border border-admin-border p-4">
      <label className="text-xs font-medium text-admin-muted">환불 사유</label>
      <input
        value={reason}
        onChange={(e) => setReason(e.target.value)}
        required
        placeholder="예: 고객 요청에 의한 계약 취소"
